...
def createPolarityDict():
    # 创建极性词典
    polarDict = {}
    with codecs.open('dict_add/sentiment_polar.txt', encoding='utf-8') as f:
        for line in f:
...
def writePairToData(readFile, writeFile, pair):
    ...
    for i in range(len(data)):
        ...
        sentiment_wordStr = ''
        sentiment_anlsStr = ''
        if type(data.loc[i, 'words']) != float:
...
if __name__ == '__main__':
    ...
    combineNegativeWordsAndSentimentWords('data/test_fc_label.csv', 'data/result.csv')

    # 获得词性标注序列
    # tagSL = getTagSequenceList("train.data", ' ')
    data_util = DataUtil()
    data = data_util.load_data1('data/result.csv')
    tagSL = []
    for i in range(len(data)):
    ...
    writePairToData("data/result.csv", "data/result.csv", pair)
    # testPairPrecision()
